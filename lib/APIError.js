var errors = {
  ...
  conversationNotFound : { code: 11, msg: 'Conversation not found' },
  notAllowed   : { code: 12, msg: 'Method not allowed' },
  invalidEmail : { code: 13, msg: 'Invalid email' },
  unauthorized : { code: 14, msg: 'Invalid password or email' },
  userExists   : { code: 15, msg: 'User already exists' },
...
var APIError = function (HTTPStatus, error, message) {
  ...
  this.handleError = function (res) {
    ...
    res.send(this.HTTPStatus, { status: this.HTTPStatus, code: this.error.code, message: this.error.msg }, res)
  }
}
